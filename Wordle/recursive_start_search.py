...
def auto_play(answer, start_guess):
    ...
    while True:
        ...
        if len(game_list) == 1:
            ...
            if result == '00000':
                zeroes_chain = game_chain[:-10] + ''.join(sorted(guess)) + result
                if zeroes_chain in big_fucking_hash:
                    guess = big_fucking_hash[zeroes_chain]
                else:
                    guess = retrieve_guess(game_list)
                    big_fucking_hash[zeroes_chain] = guess
                    big_fucking_hash[game_chain] = guess
            else:
